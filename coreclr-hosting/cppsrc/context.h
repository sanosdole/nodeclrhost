...
class Context {
    ...
    bool IsActiveContext() {
        return ThreadInstance::Current() == this;
    }
...
public:
    ...
    static Context* CurrentInstance() {
        return ThreadInstance::Current();
    }
        
    // TODO: How to get the instance when not on node thread? => .NET calls getInstance on main thread
    // TODO: How to ensure ThreadContext on calls from node to .NET delegates (see DotNetHandle)
    void PostCallback(netCallback_t callback, void* data); // can be called from any thread
    void Release(); // can be called from any thread

    // Get/SetMember , CreateObject, Invoke(Member) can only be called on node thread
    JsHandle GetMember(JsHandle owner_handle, const char* name);
    JsHandle SetMember(JsHandle owner_handle, const char* name, DotNetHandle dotnet_handle);
    JsHandle CreateObject(JsHandle prototype_function, int argc, DotNetHandle* argv);
    JsHandle Invoke(JsHandle handle, JsHandle receiver_handle, int argc, DotNetHandle* argv);
};
