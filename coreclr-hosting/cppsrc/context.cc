...
Context::Context(std::unique_ptr<DotNetHost> dotnet_host, Napi::Env env)
    : env_(env),
      ...
      function_factory_(std::bind(&Context::CreateFunction, this, std::placeholders::_1)) {
...
void Context::UvAsyncCallback() {
    ...
    if (release_called_) {
        uv_close(reinterpret_cast<uv_handle_t *>(&async_handle_), &asyncReleaseCallback);    
    }
...
Napi::Value Context::RunCoreApp(const Napi::CallbackInfo& info) {
    ...
    auto execute_result = context->host_->ExecuteAssembly(info[1].ToString(), 1, argv, exit_code);
...
JsHandle Context::SetMember(JsHandle owner_handle, const char* name, DotNetHandle dotnet_handle) {
    if (!owner_handle.IsObject())
        return JsHandle::Error("Only objects support GetMember");
    if (!IsActiveContext())
        return JsHandle::Error("Must be called on node thread");
...
JsHandle Context::CreateObject(JsHandle prototype_function, int argc, DotNetHandle* argv) {
    if (!IsActiveContext())
        return JsHandle::Error("Must be called on node thread");
    ...
    if (prototype_function.IsNotNullFunction())
    {
        ...
        napi_value result;
        auto status = napi_new_instance(env_, prototype_function.ToValue(env_), argc, arguments.data(), &result);
        if (status != napi_ok)
        {
...
JsHandle Context::Invoke(JsHandle handle, JsHandle receiver_handle, int argc, DotNetHandle* argv) {
    if (!IsActiveContext())
        return JsHandle::Error("Must be called on node thread");
