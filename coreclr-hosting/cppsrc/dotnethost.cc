...
// Define OS-specific items like the CoreCLR library's name and path elements
    ...
    void FreeLibrary(LibraryHandle library) {
        dlclose(library);
    }
...
DotNetHostCreationResult::Enum DotNetHost::Create(std::string base_path, std::unique_ptr<DotNetHost>& host) {
    ...
    auto coreclr_initialize = GetFunction<coreclr_initialize_ptr>(coreclr, "coreclr_initialize");
    if (NULL == coreclr_initialize) {
        FreeLibrary(coreclr);
        return DotNetHostCreationResult::kInvalidCoreClr;
    }

    auto coreclr_create_delegate = GetFunction<coreclr_create_delegate_ptr>(coreclr, "coreclr_create_delegate");
    if (NULL == coreclr_create_delegate) {
        FreeLibrary(coreclr);
        return DotNetHostCreationResult::kInvalidCoreClr;
    }

    auto coreclr_shutdown = GetFunction<coreclr_shutdown_ptr>(coreclr, "coreclr_shutdown");
    if (NULL == coreclr_shutdown) {
        FreeLibrary(coreclr);
        return DotNetHostCreationResult::kInvalidCoreClr;
    }

    auto coreclr_execute_assembly = GetFunction<coreclr_execute_assembly_ptr>(coreclr, "coreclr_execute_assembly");
...
DotNetHostExecuteAssemblyResult::Enum DotNetHost::ExecuteAssembly(std::string name, int argc, const char** argv, unsigned int& result_code) {
